...
## 2026-03-14 Implementation Status
Implemented in repo:
- Config flags in `config/application.rb` for `ADSENSE_ENABLED`, `ADSENSE_CLIENT_ID`, `ADSENSE_SLOT_INLINE`, `ADSENSE_SLOT_FOOTER`.
- Layout-level script injection via helper (flag-driven).
- Slot partials for inline/footer placements.
- Public-only gating via `ApplicationController#adsense_authenticated?` (checks common auth helpers if present).
- Privacy policy page with opt-out controls and GPC support (opt-out disables AdSense for that browser).
...
- Update privacy/cookie disclosures if required for your jurisdiction.
- Verify CSP if you later enable strict content security policy.
